...
from utils.model_contrasts import (
    model_contrasts, region_to_layer, layer_to_region, all_models,
    model_dirs_gen)
...
def main():
    ...
    for m, model_dir in enumerate(all_models):
        overwrite = False
        overwrite = get_model_responses(
            model_dir, m, len(all_models), overwrite=overwrite)
        overwrite = RSA_fMRI(
            model_dir, m, len(all_models), overwrite=overwrite)
        if overwrite:
...
def get_model_responses(model_dir, m=0, total_models=0, overwrite=False):
    ...
    if not op.isfile(responses_path) or overwrite:
        ...
        activations = get_activations(
            model, model_name, images, batch_size=24,
        ...
        for occluder in fMRI.occluders:
            ...
            for exemplar in fMRI.exemplars:
                ...
                paths += sorted(glob.glob(f'{out_dir}/{exemplar}_'
                                          f'{occluder}_cyc*.png'))
            tile(paths, tiled_recons.replace('.svg', f'_{occluder}.png'),
        ...
        for analysis, params in fMRI.occlusion_robustness_analyses.items():
            ...
            if collate_data:
                ...
                for subtype in df.subtype.unique():
                    ...
                    outpath = f'{out_dir}/{params["index_label"]}_{subtype}.svg'
