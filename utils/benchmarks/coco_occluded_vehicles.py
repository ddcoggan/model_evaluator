...
class COCO_Occluded_Vehicles_Dataset(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        for cls, trg in CLASSES.items():
            paths = sorted(glob.glob(f'{self.main_dir}/{cls}_occ/*.jpg'))
...
def crop_images():
    for image_path in sorted(glob.glob(f'{BENCHMARK_BASE}/*_occ/*.jpg')):
        print(image_path)
        image = Image.open(image_path).convert("RGB")
        if image.size != (224, 224):
            image_resized = image.resize((  # resize so shortest side is 224
                int(224 * image.size[0] / min(image.size)),
                int(224 * image.size[1] / min(image.size))))
            image_cropped = image_resized.crop((  # center crop to 224x224
                int((image_resized.size[0] - 224) / 2),
                int((image_resized.size[1] - 224) / 2),
                int((image_resized.size[0] + 224) / 2),
                int((image_resized.size[1] + 224) / 2)))
...
def score_model(model_dir, architecture, batch_size, m=0, total_models=0,
    ...
    if not results.empty and len(results[results.benchmark == BENCHMARK]) == 2:
        return False
    ...
    dataset = COCO_Occluded_Vehicles_Dataset(transform=transform)
    def collate_fn(data):
